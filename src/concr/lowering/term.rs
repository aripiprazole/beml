...
impl LoweringCtx {
    pub fn do_lowering(mut self, term: Term) -> miette::Result<abstr::Term> {
        self.burn();

        match term {
            SrcPos(box term, loc) => {
                self.src_pos = loc.clone();
                Ok(abstr::SrcPos(self.do_lowering(term)?.into(), loc))
            }
    ...
    pub fn parse_type(mut self, term: Term) -> miette::Result<abstr::Type> {
        match term {
            SrcPos(box term, loc) => {
                self.src_pos = loc.clone();
                Ok(abstr::Type::SrcPos(self.parse_type(term)?.into(), loc))
            }
    ...
    pub fn parse_parameter(&mut self, parameter: Term) -> miette::Result<(abstr::Pattern, abstr::Type)> {
        self.burn();
