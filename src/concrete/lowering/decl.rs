    ...
    pub fn do_declaration_lowering(&mut self, term: Term) -> miette::Result<Defer> {
        Ok(match term {
            ...
            TypeDecl(TypeDecl {
                ...
                Defer(Box::new(|_| {
                    Ok(abs::Decl::TypeDecl(abs::TypeDecl {
                        name,
                        variables,
                        cases,
                        loc: src_pos,
            ...
            LetDecl(LetDecl {
                ...
                Defer(Box::new(|ctx| {
                    ...
                    if let abs::Variable(variable) = pattern {
                        Ok(abs::Decl::LetDecl(abs::LetDecl {
                            name: variable,
                            type_repr: fun_type,
                            body: abs::Value(body),
                            loc: src_pos,
                        ...
                        Ok(abs::Decl::LetDecl(abs::LetDecl {
                            ...
                            type_repr: fun_type,
                            body: abs::Value(abs::Match(body.into(), vec![abs::Case {
                                pattern,
                                body: abs::Var(ctx.new_fresh_variable().use_reference()),
                            }])),
                            loc: src_pos,
                        }))
                    }
